# The Holistic Implementation Model — UDLM's place in it
...
- **Platform** — can the architecture/plan support the UC? (capabilities,
  specification, rules, context, execution). DCM is a reference implementation.
...
> **Platform + People/Process + Enablement → implementation of Use Cases → the holistic vision.**
...
UDLM models data **from intent to realization** in a implementation-neutral, wire-compatible
way. In the holistic model, the pillars (Platform, People/Process, Enablement) are
*implementations*; UDLM is the **substrate that represents their data universally** so that
...
**DAV is a UDLM implementation — a peer to DCM** (decision 2026-06-08). DCM realizes UDLM
...
The model's spine — **intent (Use Cases / desired outcomes) → implementation (the three
...
one coherent, universal substrate. UDLM stays implementation-neutral: any pillar tool that
