...
- **Data scoping is preserved across the split.** Each verb is an ordinary governed DCM→Provider crossing carrying only the `role: execution` slice; the Governance Matrix fires at every crossing (`docs/spec/contracts/data-roles.md`; `PRV-008`; provider-contract §6a). The two-phase model adds request *types*, not new data-exposure paths.
